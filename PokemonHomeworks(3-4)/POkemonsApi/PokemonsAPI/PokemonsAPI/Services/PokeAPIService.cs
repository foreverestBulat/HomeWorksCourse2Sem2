...
public static class PokeAPIService
{
    ...
    public static async Task HasPokemonsAndRelatedEntities(this ApplicationDbContext context, string url = "https://pokeapi.co/api/v2/pokemon?offset=0&limit=45")
    {
        //if (context.Pokemons.Count() > 0) return;
    ...
    public static async Task HasAbilities(this ApplicationDbContext context, string url = "https://pokeapi.co/api/v2/ability?limit=50") // 367
    {
        //if (context.Abilities.Count() > 0) return null;
        var result = JsonConvert.DeserializeObject<ResultList>(await GetBody(url));
        ...
        foreach (var item in result.Results)
        {
            ...
            //abilities.Add(new Ability() { Name = item.Name });
            //builder.Entity<Ability>().HasData(new Ability() { Name = item.Name });
    ...
    public static async Task HasMoves(this ApplicationDbContext context, string url = "https://pokeapi.co/api/v2/move?limit=100")
    {
        //if (context.Moves.Count() > 0) return;
        ...
        foreach (var item in result.Results)
        {
            ...
            //builder.Entity<Move>().HasData(new Move() { Name = item.Name, Type = context.PokemonTypes.Where(e => e.Name == resultMove.Type.Name).FirstOrDefault() });
    ...
    public static async Task HasPokemonTypes(
            this ApplicationDbContext context, 
            string url = "https://pokeapi.co/api/v2/type?limit=100"
        )
    {
        if (context.PokemonTypes.Count() > 0) return;
        var result = JsonConvert.DeserializeObject<ResultList>(await GetBody(url));
        ...
        foreach (var item in result.Results)
        {
            ...
            //builder.Entity<PokemonType>().HasData(new PokemonType() { Name = item.Name });
